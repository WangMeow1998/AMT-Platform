package com.meow.community.event;
...
public class EventConsumer implements CommunityConstant {
    ...
    public void handleMessage(ConsumerRecord record){
        ...
        message.setFromId(SYSTEM_USER_ID); //发送通知者是系统管理员
        message.setToId(event.getEntityUserId()); //接受通知者是实体所对应的用户
        message.setConversationId(event.getTopic());
        message.setCreateTime(new Date());

        Map<String, Object> content = new HashMap<>();
        content.put("userId", event.getUserId()); //userId是事件触发者的Id
        content.put("entityType", event.getEntityType()); //实体类型
        content.put("entityId", event.getEntityId()); //实体的Id

        if(!event.getData().isEmpty()){ //如果event中还有其他数据，也要把他们加入content中
